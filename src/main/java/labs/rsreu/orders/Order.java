...
//    private final Client client;
    ...
    private final List<Consumer<TransactionInfo>> statusCallbacks = new ArrayList<>();
     ...
     */
    public Order(OrderType type, int clientId, CurrencyPair currencyPair, BigDecimal amountFirst, BigDecimal amountSecond) {
        this.id = idCounter++;
        this.type = type;
...
//        this.client = other.client;
     ...
     */
    public void addStatusCallback(Consumer<TransactionInfo> callback) {
        statusCallbacks.add(callback);
    }

    /**
     * Создаем уведомеление
     * @param transactionInfo - уведомление
     */
    public void notifyStatus(TransactionInfo transactionInfo) {
        for (Consumer<TransactionInfo> callback : statusCallbacks) {
            callback.accept(transactionInfo);
        }
    }
}
